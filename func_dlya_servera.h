...
private:
    ...
    static QByteArray mtd_dln_ppl(const QString arr, QString target); // метод деленяи пополам
    static QByteArray graf(QString vert, QString edg, QString start_end);
    static std::vector<std::tuple<int, int, int>> parseEdgeString(const QString& edgeString);// граф граф граф граф граф
};
